...
/****************************************************************************************/
PS2X::PS2X() : m_globalTolerance(0) {
    initTolerance(m_LXtolerance);
    initTolerance(m_LYtolerance);
    initTolerance(m_RXtolerance);
    initTolerance(m_RYtolerance);
}
...
/****************************************************************************************/
void PS2X::printCheckError(byte error) {
    if(error == 0){
        Serial.print("Controleur trouve, configuration reussi ");
    }
    /*
    ...
    */
        ...
        Serial.println("Controleur non trouve, verifier les branchements, see readme.txt to enable debug. visit www.billporter.info for troubleshooting tips");

    else if(error == 2)
        Serial.println("Controleur trouve mais n'accepte pas les commands. see readme.txt to enable debug. Visit www.billporter.info for troubleshooting tips");

    else if(error == 3)
        Serial.println("Le controleur refuse d'entrer dans le mode de pression (pressures), il ne le supporte peut etre pas. ");
}

/****************************************************************************************/
void PS2X::printCheckControllerType(byte type) {
    switch(type) {
        case 0:
            Serial.print("Type de controleur trouve non connu ");
            break;
        case 1:
            Serial.print("DualShock Controleur trouve ");
            break;
        case 2:
            Serial.print("GuitarHero Controleur trouve ");
            break;
        case 3:
            Serial.print("Sony DualShock Controller sans-fil trouve ");
            break;
    }
}

/****************************************************************************************/
void PS2X::setAxisTolerance(byte axis, byte lowerBound, byte upperBound) {
    switch (axis) {
        case PSS_LX:
            m_LXtolerance.lower = lowerBound;
            m_LXtolerance.upper = upperBound;
            break;
        case PSS_LY:
            m_LYtolerance.lower = lowerBound;
            m_LYtolerance.upper = upperBound;
            break;
        case PSS_RX:
            m_RXtolerance.lower = lowerBound;
            m_RXtolerance.upper = upperBound;
            break;
        case PSS_RY:
            m_RYtolerance.lower = lowerBound;
            m_RYtolerance.upper = upperBound;
            break;
    }
}

/****************************************************************************************/
void PS2X::setAllAxisTolerance(byte lowerBound, byte upperBound) {
    setAxisTolerance(PSS_LX, lowerBound, upperBound);
    setAxisTolerance(PSS_LY, lowerBound, upperBound);
    setAxisTolerance(PSS_RX, lowerBound, upperBound);
    setAxisTolerance(PSS_RY, lowerBound, upperBound);
}

/****************************************************************************************/
void PS2X::setAllAxisTolerance(byte tolerance) {
    m_globalTolerance = tolerance;
}

/****************************************************************************************/
byte PS2X::getMinBound(byte const axis) {
    if (m_LXtolerance.lower != 0 || m_LYtolerance.lower != 0 || m_RXtolerance.lower != 0 || m_RYtolerance.lower != 0) {
        switch (axis) {
            case PSS_LX:
                return m_LXtolerance.lower;
                break;
            case PSS_LY:
                return m_LYtolerance.lower;
                break;
            case PSS_RX:
                return m_RXtolerance.lower;
                break;
            case PSS_RY:
                return m_RYtolerance.lower;
                break;
        }
    }
    else if (m_globalTolerance != 0) {
        return 127 - m_globalTolerance;
    }
    else
        return 127;
}
...
/****************************************************************************************/
byte PS2X::getMaxBound(byte const axis) {
    if (m_LXtolerance.lower != 0 || m_LYtolerance.lower != 0 || m_RXtolerance.lower != 0 || m_RYtolerance.lower != 0) {
        switch (axis) {
            case PSS_LX:
                return m_LXtolerance.upper;
                break;
            case PSS_LY:
                return m_LYtolerance.upper;
                break;
            case PSS_RX:
                return m_RXtolerance.upper;
                break;
            case PSS_RY:
                return m_RYtolerance.upper;
                break;
        }
    }
    else if (m_globalTolerance != 0) {
...
void PS2X::initTolerance(BoundsTolerance &tolerance) {
    tolerance.lower = 0;
    tolerance.upper = 0;
}
